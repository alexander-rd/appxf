...
class Security():
    ...
    def init_user(self, password):
        ...
        A key is derived from the password (derived key). A secret key, not tied
        to the pasword is generated (secret key). Then, the derived key is used
        to persist the secret key on the file systen. The password is not
        stored.
        ...
        self.__encrypt_to_file(derived_key, self._user_secret_key, self._user_key_file)
    ...
    def unlock_user(self, password):
        ...
        if not self.is_user_initialized():
            raise Exception('User is not initialized. Run init_user() if file {0} was lost.'.format(self._user_key_file))
        derived_key           = self.__derive_key(password)
        self._user_secret_key = self.__decrypt_from_file(derived_key, self._user_key_file)

    def encrypt_to_file(self, data, file):
        if not self.is_user_unlocked():
            raise Exception('Trying to encrypt {0} before succeeding with unlock_user()'.format(file))
        self.__encrypt_to_file(self._user_secret_key, data, file)
            
    def __encrypt_to_file(self, key, data, file):
        # get file where file is allowed to be a list of strings to avoid caller
        # needing to use os.path:
        if not type(file) == list: file = [file]
    ...
    def decrypt_from_file(self, file):
        if not self.is_user_unlocked():
            raise Exception('Trying to decrypt {0} before succeeding with unlock_user()'.format(file))
    ...
    def __decrypt_from_file(self, key, file):
        ...
        # validated before returning it back to the caller (like using a hash on the
        # data)
