...
from tests._fixtures.env_storage import env_test_directory
...
def test_fileversions_existing(env_test_directory):
    ...
    assert fileversions.get_filename(
        'file_(00).txt', directory=env['dir'], existing=True) is None
    assert fileversions.get_filename(
        'file_00.txt', directory=env['dir'], existing=True) is None
    # Drop file
    open(os.path.join(env['dir'], 'file_00.txt'), 'w').close()
    # Check if now existing:
    assert fileversions.get_filename(
        'file_00.txt', directory=env['dir'], existing=True) == 'file_00.txt'
    assert fileversions.get_filename(
        'file_(00).txt', directory=env['dir'], existing=True) == 'file_00.txt'
    # Check if next version is correct
    assert fileversions.get_filename(
        'file_(00).txt', directory=env['dir']) == 'file_01.txt'
