...
# TODO: this locale setting is actually a KISS language setting. But it is not
# clear how to model this.
#
...
# 3) Kiss central module. This could also hold other "central" elements like a
#    kiss_cf logger such that this one can be configured instead of the root
#    logger. When importing kiss_cf, we would gain access to it (and all sub
#    modules)
...
#  * implementing in kiss_cf.language.babel
#  * allow access to set it from kiss_cf, language or any other using module
#    like this one, wrapping kiss_cf.language.babel.set_locale().
#
...
def get_filename(format: str,
    ...
    filename. All date formatting from babel can be used. kiss_cf identifies
