...
class EditConfigWindow(tkinter.Toplevel):
    def __init__(self, config: Config, section: str, title='Settings for {0}'):
        ...
        def cancelButtonFunction(event=None):
            ...
            config.restore()
            guiRoot.destroy()    
        cancelButton = tkinter.Button(buttonFrame, text = translate(config.language, 'Cancel'), command=cancelButtonFunction)
        ...
        okButton = tkinter.Button(buttonFrame, text = translate(config.language, 'OK'), command=okButtonFunction)
    ...
    def __init__(self, config: Config):
        ...
        for section in config.config.sections():
            ...
            def command(section=section): 
                window = EditConfigWindow(self._config, section, title = 'Settings for {0}')
                window.grab_set()
