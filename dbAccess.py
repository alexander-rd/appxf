...
#
# The class has a single refresh() which queries all required data from the data
# base and stores it with a timestamp (encrypted) to the storage. This enables:
...
#    the SQL querry.
# 3. This class does not store data (separation of concerns). If querries are to
#    be limited
...
# Setup: https://mariadb.com/de/resources/blog/how-to-connect-python-programs-to-mariadb/
# Connector API: https://mariadb-corporation.github.io/mariadb-connector-python/index.html
...
# TODO: this one must remain private!!
# Note that sqlQuerry should be put in "" since SQL should use '' for string literals.
def getDbRawData(dbConfig, pickleName, sqlQuerry):
  # TODO: Load, if possible should change to load if DB access fails or switched
  # to offline mode.
