...
class Login():
    ...
    password. Secondary is collecting user information on first usage of a tool.
    Therefore, a configuration object defining expected user data is the main
    input.
    
    Output of the login procedure is a configuration object with the stored user
    data and the following added to it:
    ...
    def __run_init_gui(self):
        ...
        guiRoot.update()
        left_min_size_login = max([pwdLabel.winfo_width(), pwdRepLabel.winfo_width()]) + 10
        ...
        def okButtonFunction(event=None):
            ...
            if (len(pwdEntry.get()) < self._pwd_min_length):
                print('NOK, Passwort muss mindestend {0} Zeichen haben'.format(self._pwd_min_length))
